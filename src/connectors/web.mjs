...
function normalize(items, includeImages=false){
  ...
  for(const r of items||[]){
    ...
    out.push({
      id:url,
      type:includeImages?'media':'web',
      title:r.title||url,
      description:r.content||r.description||r.snippet||'',
      url,
      thumbnail:r.image_url||r.thumbnail_url,
      source:includeImages?'Image Search':'Web Search',
      sourceType:includeImages?'image':'web',
...
async function searchTavily({query,allowedDomains=[],maxResults=8,includeImages=false,timeoutMs=12000}){
  ...
  if(!r.ok){const text=await r.text().catch(()=> '');throw new Error(`Tavily API ${r.status}${text?`: ${text.slice(0,180)}`:''}`)}
  ...
  return{connector:includeImages?'images':'web',status:'ok',results:normalize(results,includeImages)};
}
